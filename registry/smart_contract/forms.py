...
class CommentEditForm(forms.ModelForm):
    ...
    class Meta:
        model = Comment
        fields = [
            'verifier', 'comment_for_rating', 'comment_for_rating_competence', 'comment_for_rating_employee',
            'competence', 'customer_flag', 'implementer_flag',
            'init_user', 'implementer', 'files', 'rating', 'employee', 'another_employee', 'rating_competence',
            'rating_employee'
        ]
        widgets = {
...
class CheckCompanyForm(forms.Form):
    TIN_or_PSRN = forms.IntegerField(label='ИНН/ОГРН', help_text='ИНН содержит 10 чисел, ОГРН - 13 чисел',
                                     widget=forms.NumberInput(
                                         attrs={'class': 'form-control', 'placeholder': 'ИНН/ОГРН'}))
...
class EditCompanyForm(forms.ModelForm):
    class Meta:
        model = Company
        fields = ['logo', 'description', 'bank', 'corr_account', 'date_of_establishment']
...
class RegistrationAceptUserForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'role': forms.Select(attrs={'class': 'select2 form-control required','style': 'width: 100%'}),
